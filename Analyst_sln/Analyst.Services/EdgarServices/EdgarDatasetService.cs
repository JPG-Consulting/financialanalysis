...
namespace Analyst.Services.EdgarServices
{
    ...
    public class EdgarDatasetService: IEdgarDatasetService
    {
        ...
        public EdgarDataset ProcessDataset(int id)
        {
            ...
            EdgarDataset ds = repository.GetDataset(id);
            LoadNumRelatedData(ds);
            LoadNums(ds);

            return ds;
        }

        private void LoadNums(EdgarDataset ds)
        {
            ...
            EdgarTaskState state = new EdgarTaskState(ds);
            numService.ProcessNums(state,subs,tags,dims);
        }

        private void LoadNumRelatedData(EdgarDataset ds)
        {
            int taskAmount = 3;
            EdgarTaskState[] states = new EdgarTaskState[taskAmount];
            states[0] = new EdgarTaskState(ds);
            states[1] = new EdgarTaskState(ds);
            states[2] = new EdgarTaskState(ds);
